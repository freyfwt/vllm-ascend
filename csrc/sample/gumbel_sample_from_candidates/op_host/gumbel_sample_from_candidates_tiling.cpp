...
static ge::graphStatus TilingForGumbelSampleFromCandidates(gert::TilingContext* context)
{
    ...
    uint32_t candidateCapacity = static_cast<uint32_t>(logitsStorageShape.GetDim(1));
    OPS_CHECK(batchSize == 0 || candidateCapacity == 0,
              OPS_LOG_E(nodeName, "batch and candidate capacity must be greater than 0."),
              return ge::GRAPH_FAILED);

    auto seedsStorageShape = seedsShape->GetStorageShape();
    uint32_t requestCount = seedsStorageShape.GetDimNum() == 0
                                ? 1U
                                : static_cast<uint32_t>(seedsStorageShape.GetDim(0));
    ...
    OPS_LOG_D(nodeName, "GumbelSampleFromCandidates B=%u C=%u tiles=%u cores=%u workspace=%lu",
              batchSize, candidateCapacity, numTiles, usedCoreNum, workspaceSizes[0]);
