...
def test_npu_map_and_record():
    ...
    expert_load = torch.zeros(3, dtype=torch.int64, device="npu")

    physical_ids = backend.map_and_record(
        topk_ids,
        logical_map,
        replica_count,
        expert_load,
    ...
    torch.testing.assert_close(expert_load.cpu(), torch.tensor([1, 1, 2], dtype=torch.int64))
