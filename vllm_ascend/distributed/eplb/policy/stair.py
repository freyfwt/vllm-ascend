...
def _build_incremental_candidate(
    ...
    return build_greedy_candidate(logical_load, current_placement, resolved_topology, deadline=deadline)
...
class StairEplbPolicy:
    ...
    def _search_layer_plan(
        ...
        search_deadline = min(
            cycle_deadline,
            time.perf_counter() + constants.MAX_SEARCH_MS_PER_LAYER / 1000,
        )
        ...
        def evaluate(counts: np.ndarray, deadline: float) -> SearchCandidate | None:
            ...
            if time.perf_counter() >= deadline or np.array_equal(placement, current):
                return None
            current_score = compute_balance_metrics(layer_load, current)
    ...
    def plan_rebalance(
        ...
        cycle_deadline = start_time + constants.MAX_PLANNER_MS / 1000
